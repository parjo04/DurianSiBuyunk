<?php
/**
 * Stock Management page for Tasikmalaya branch
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'update_stock') {
        ...
        if ($operation === 'add') {
            $result = addStockWithWeight($produk_id, 'tasik', $bobot_kg, $jumlah_pcs, $keterangan, $_SESSION['user']['id']);
        } else {
            $result = reduceStockWithWeight($produk_id, 'tasik', $bobot_kg, $jumlah_pcs, $keterangan, $_SESSION['user']['id']);
        }
...
$products = getProducts(CABANG_TASIK, $kategori_id, $search);

include __DIR__ . '/../../../includes/header.php';
?>

<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><i class="fas fa-warehouse"></i> Manajemen Stok - Tasikmalaya</h1>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <?php foreach ($products as $product): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($product['harga_per_kg'] && $product['total_kg_tasik']): ?>
                                        <span class="fw-bold text-dark">
                                            <?= formatRupiah($product['harga_per_kg'] * $product['total_kg_tasik']) ?>
                                        </span>
